...
Commerce = tab.excel_ref('B').expand(DOWN).by_index([5,66,126]) - tab.excel_ref('B182').expand(DOWN)

Modeoftransport = tab.excel_ref('B').expand(DOWN).by_index([7,31,45,51,57,59,61,63,68,95,103,105,111,113,117,
                                                            119,121,123,128,132,139,145,147,153,152,154,164,170,176,178,180,182])
                                                                

Modeoftransport = Modeoftransport - tab.excel_ref('B182').expand(DOWN)

Services = tab.excel_ref('B').expand(DOWN).by_index([9,16,20,25,27,29,33,34,36,37,39,41,43,63,70,77,81,85,87,89,93,95,97,105,
                                                     125,130,154,162,182]) - tab.excel_ref('B182').expand(DOWN)
